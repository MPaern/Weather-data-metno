...
def find_index_of_closest_datapoint(
    ...
    # Find intex of closest point in the longitude (and latitude) grid
    # NOTE: a possible error that might occure here is that multiple points in
    # the grid may have the same longitude, causing more than one point to be
    # returned. This sometimes happends when using the latutude instead of the
    # lonitude grid (but this issue have not been observed when using 'lon').
